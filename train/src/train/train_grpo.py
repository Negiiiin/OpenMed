...
def train():
    ...
    model_id_path = pathlib.Path(model_args.model_id)
    is_peft_adapter_dir = model_id_path.is_dir() and (model_id_path / "adapter_config.json").exists()
    processor_model_id = model_args.model_id
    if is_peft_adapter_dir:
        raise RuntimeError(
            f"model_id '{model_args.model_id}' is a PEFT adapter directory. "
            "On-the-fly LoRA merging is incompatible with DeepSpeed ZeRO-3 training. "
            "Pre-merge the adapter first with scripts/merge_checkpoint.sh, "
            "then set the merged directory as --model_id."
        )
